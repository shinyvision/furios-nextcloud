...
func NewWindow(app *gtk.Application, debugMode bool) *gtk.ApplicationWindow {
	...
	appIcon.SetMarginEnd(10)
	header.Append(appIcon)

	logoLabel := gtk.NewLabel("nextcloud")
	logoLabel.AddCSSClass("header-logo")
	logoLabel.SetHExpand(true)
	logoLabel.SetHAlign(gtk.AlignStart)
	header.Append(logoLabel)
	...
	filesPage := pages.NewFilesPage(showPage, func() { toggleMenu(true) })
